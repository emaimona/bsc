...
(--) (x1,y1,z1) (x2,y2,z2) = toReal ( ((toInt(x2 - x1))^2 + (toInt(y2 - y1))^2 + (toInt(z2 - z1))^2)^(toInt(1/2)) )


distance :: (Real, Real, Real) [(Real, Real, Real)] -> [Real]
distance tup lis = [x--tup \\ x<-lis]
...
remV :: String -> String
remV str = {s \\ s<-:str | not(isMember s vowels)}

removeVowels :: {{String}} -> {{String}}
removeVowels arr = {{remV a.[0], remV a.[1]} \\ a<-:arr}
...
FindLinesC st t = length [1 \\l<-Linelist | l.transport == t && isMember st l.stops ] >= 1
...
FindLinesR st = [(l.number,l.transport) \\l<-Linelist |isMember st l.stops]
...
FindLinesB st1 st2 = [(l.transport, l.number) \\l<-Linelist |isMember st1 l.stops && isMember st2 l.stops]
...
FindLinesT st1 st2 t = [ l.number \\l<-Linelist |isMember st1 l.stops && isMember st2 l.stops && l.transport == t]
...
auxAge :: Int Planet -> Int
auxAge age Mercury = (age*365/88)
auxAge age Venus = (age*365/225) //225 days
auxAge age Mars = (age*365/687) //687 days
auxAge age Jupiter = tInt(toReal(age)/11.4) //11.8 years
auxAge age Saturn = tInt(toReal(age)/29.4) //29.4 years
auxAge age _ = abort "It is not a plannet"
...
ageOn :: {Person} -> {Int}
ageOn arr = {auxAge p.age p.planet \\ p<-:arr}
...
treeL ::  (TreeT Int) -> [Int]
treeL LeafT = []
treeL (NodeT a le ri) = treeL le ++ [a] ++ treeL ri

isGood :: (TreeT Int) -> Bool
isGood LeafT = True
isGood (NodeT a le ri)
| avg (treeL (NodeT a le ri)) == a = isGood le && isGood ri
= False


goodNodes :: (TreeT Int) -> Int
goodNodes LeafT = 0
goodNodes (NodeT a le ri)
| isGood (NodeT a le ri) = 1 + goodNodes le + goodNodes ri
...
exactly3children (Node a le mid ri) = isNode le && isNode mid && isNode ri

count :: (Tree a) -> Int
count Leaf = 0
count (Node a le mid ri)
| exactly3children (Node a le mid ri) = 1 + count le + count mid + count ri
= count le + count mid + count ri
...
isNode2 :: (Tree2 a) -> Bool
isNode2 Leaf2 = False
isNode2 _ = True

onlyRight :: (Tree2 a) -> Bool
onlyRight (Node2 a le ri) = not(isNode2 le) && isNode2 ri

auxBig :: (Tree2 Int) Int -> Int
auxBig Leaf2 m = m
auxBig  (Node2 a le ri) m
| onlyRight (Node2 a le ri) = max (auxBig le (max a m)) (auxBig ri (max a m))
=  max (auxBig le m) (auxBig ri m)

biggestNode :: (Tree2 Int) -> Int
biggestNode t = auxBig t -9999999
...
= Nothing
where
	holds1 = (f1 (fst t)) && (f2 (snd t))
	holds2 = (f1(fst t)) && not(f2 (snd t))
	holds3 = not(f1 (fst t)) && (f2 (snd t))
	pr = fst t * snd t
	sm = fst t + snd t
