...
class Article {
    ...
    private void printReference(Book book, int from, int to) {
        String refs = book.createReference(this.title, from, to);
        pw.println(refs);
    }

    public void print(String destinationFileName) {
        try {
            pw = new PrintWriter(new File(destinationFileName));
            pw.println(title);
            pw.println(body);
...
class MainBook {
    public static void main(String[] args) {
        ...
        Article myArticle = new Article("My Article title", "the body", "the conclusion");
        myArticle.addBookToReferences(book1);
        myArticle.addBookToReferences(pb);
        myArticle.addBookToReferences(ebook);

        myArticle.print("outfile.txt");
